...
//        println("?????????????????========" + params)
        ...
        if (stuList[0]) {
            ...
            def subjectSessionList = CourseSubject.findAllBySemesterAndProgramSession(Semester.findBySemesterNoAndProgramSession(stuList[0].semester, stuList[0].programSession), programSessionIns)*.subjectSessionId

            def dateList = ['Exam Date'], timeList = ['Exam Time']
            if(subjectSessionList){
